...
{
    public partial class Stationfsc : Page
    {
        ...
        [WebMethod]
        public static int GET_JOB_COUNT()
        {
            try
            {
                using (TMdbEntities mdbEntities = new TMdbEntities())
                {
                    var res = mdbEntities.SEAT_DATA.Where(i => i.FSC == null && i.BuildNoDatetime > DateTime.Today).ToList();
                    if (res.Count > 0)
                    {
                        return res.Count;
                    }
        ...
        public static string IsQRValid(string build_ticket)
        {
            try
            {
                ...
                {
                    var res = dbEntities.SEAT_DATA.Where(i => i.BuildLabelBarcode == build_ticket && i.FSC == null).OrderByDescending(o => o.ID).FirstOrDefault();
                    if (res != null)
                    {
                        if (res.STAUS == "REJECT")
                        {
        ...
        [WebMethod]
        public static string BuildTicketExecuteTask(int id, string val, string model_variant)
        {
