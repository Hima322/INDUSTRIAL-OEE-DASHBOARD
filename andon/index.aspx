...
<head runat="server">
    ...
    <script>
                    ...
                    if (res.d != "Error") {
                        ...
                        document.getElementById("andon_data").innerHTML =
                            data.map(e => {
                                ...
                                let startDateParse = Date.parse(new Date().toISOString().split('T')[0] + " " + String(e.HourName.split("-")[0]))
                                let endDateParse = Date.parse(new Date().toISOString().split('T')[0] + " " + String(e.HourName.split("-")[1]))

                                var a = moment(Date.now()).isAfter(startDateParse)
                                var b = moment(Date.now()).isBefore(endDateParse)
        ...
        const getCurrentShiftName = () => {
            $.ajax({
                type: "POST",
                url: "index.aspx/GetCurrentShiftName",
                data: "",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: "true",
        ...
        const GetCurrentShiftRowId = () => {
            $.ajax({
                type: "POST",
                url: "index.aspx/GetCurrentShiftRowId",
                data: "",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: "true",
        ...
        const getAllAndonDetails = () => {
            $.ajax({
                type: "POST",
                url: "index.aspx/GetAllAndonDetails",
                data: "",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: "true",
